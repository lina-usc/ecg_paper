...
def params_array_to_dict(params, log_labels=None):
    ...
    return {log_label: dict(zip(["mu", "sigma", "t0", "D"], row))
            for row, log_label in zip(params.reshape(len(params) // 4, 4), log_labels)}


def compute_snr(params, t, mean_beat):
    sim_beat = np.sum(np.array([lognpdf(t, **ln) for ln in params.values()]), axis=0)
    return 10 * np.log10(np.trapz(mean_beat ** 2) / np.trapz((mean_beat - sim_beat) ** 2))
...
class SigmaLog:
    ...
    def __op__(self, other, fct):
        ...
        if isinstance(other, (SigmaLog, dict)):
            ret = {ln: {p: getattr(float(self.params[ln][p]), fct)(other.params[ln][p])
...
def plot_fit_heart_beat(params, t, mean_beat, lower, upper,
    ...
    if show_bounds:
        ubs, lbs = np.array([get_bounds(t, param_lower, param_upper)
                             for param_lower, param_upper in zip(lower.values(), upper.values())]).transpose([1, 0, 2])
    ...
    ax.set_title(
        f"SNR: {np.round(10 * np.log10(np.trapz(mean_beat ** 2) / np.trapz((mean_beat - sim_beat) ** 2)), 2)}dB")


def lognpdf3(t, mu=0.0, sigma=1.0, t0=0.0):
    return lognorm(s=sigma, loc=t0, scale=np.exp(mu)).pdf(t)


def get_upper_bound(time, param_lower, param_upper):
    """
     Equation 13 from https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=6183528
    """
    upper_bound = np.zeros_like(time)

    masks = [time > param_lower["t0"],
             time > param_lower["t0"] + np.exp(param_lower["mu"] - param_upper["sigma"]),
             time > param_lower["t0"] + np.exp(param_lower["mu"] - param_lower["sigma"]),
             time > param_lower["t0"] + np.exp(param_lower["mu"] - param_lower["sigma"] ** 2),
             time > param_upper["t0"] + np.exp(param_lower["mu"] - param_lower["sigma"] ** 2),
             time > param_upper["t0"] + np.exp(param_lower["mu"]),
             time > param_upper["t0"] + np.exp(param_upper["mu"]),
             time > param_upper["t0"] + np.exp(param_upper["mu"] + param_lower["sigma"]),
             time > param_upper["t0"] + np.exp(param_upper["mu"] + param_upper["sigma"]),
             np.array(False)]
    masks = [m1 & ~m2 for m1, m2 in zip(masks[:-1], masks[1:])]

    upper_bound[masks[0]] = lognpdf3(time[masks[0]], t0=param_lower["t0"],
                                     mu=param_lower["mu"], sigma=param_upper["sigma"])
    upper_bound[masks[1]] = lognpdf3(time[masks[1]], t0=param_lower["t0"],
                                     mu=param_lower["mu"],
                                     sigma=param_lower["mu"] - np.log(time[masks[1]] - param_lower["t0"]))
    upper_bound[masks[2]] = lognpdf3(time[masks[2]], t0=param_lower["t0"],
                                     mu=param_lower["mu"], sigma=param_lower["sigma"])
    upper_bound[masks[3]] = np.ones_like(time[masks[3]]) * lognpdf(
        np.exp(param_lower["mu"] - param_lower["sigma"] ** 2),
        t0=0, mu=param_lower["mu"],
        sigma=param_lower["sigma"])
    upper_bound[masks[4]] = lognpdf3(time[masks[4]], t0=param_upper["t0"],
                                     mu=param_lower["mu"], sigma=param_lower["sigma"])
    upper_bound[masks[5]] = lognpdf3(time[masks[5]], t0=param_upper["t0"],
                                     mu=np.log(time[masks[5]] - param_upper["t0"]), sigma=param_lower["sigma"])
    upper_bound[masks[6]] = lognpdf3(time[masks[6]], t0=param_upper["t0"],
                                     mu=param_upper["mu"], sigma=param_lower["sigma"])
    upper_bound[masks[7]] = lognpdf3(time[masks[7]], t0=param_upper["t0"],
                                     mu=param_upper["mu"],
                                     sigma=np.log(time[masks[7]] - param_upper["t0"]) - param_upper["mu"])
    upper_bound[masks[8]] = lognpdf3(time[masks[8]], t0=param_upper["t0"],
                                     mu=param_upper["mu"], sigma=param_upper["sigma"])
    return upper_bound


def get_lower_bound(time, param_lower, param_upper):
    """
     Equation 22 from https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=6183528
    """

    t0s = np.array([param_upper["t0"]] * 2 + [param_lower["t0"]] * 3)[:, None]
    mus = np.array([param_upper["mu"]] * 3 + [param_lower["mu"]] * 2)[:, None]
    sigmas = np.array([param_lower["sigma"]] + [param_upper["sigma"]] * 3 + [param_lower["sigma"]])[:, None]
    return lognpdf3(time, t0=t0s, mu=mus, sigma=sigmas).min(0)


def get_bounds(time, param_lower, param_upper):
    assert (np.all([param_lower[param] < param_upper[param] for param in param_lower]))
    lower_bound = get_lower_bound(time, param_lower, param_upper)
    upper_bound = get_upper_bound(time, param_lower, param_upper)

    return (param_upper["D"] * upper_bound if param_upper["D"] > 0 else param_upper["D"] * lower_bound,
            param_lower["D"] * lower_bound if param_lower["D"] > 0 else param_lower["D"] * upper_bound)
...
class SigmaECGEnv(Env):
    ...
    def __init__(self, segment_df, lower, upper, prototype):
        ...
        self.action_space = Box(-0.01, 0.01, shape=params_dict_to_array(self.upper.params).shape)
        spaces = {
            "error": Box(low=np.ones_like(self.t) * 1.5 * v_min,
                         high=np.ones_like(self.t) * 1.5 * v_max, dtype=np.float64),
            "estimate": Box(self.lower.to_array(), self.upper.to_array(), dtype=np.float64),
        }
    ...
    def action_to_beat(self, action):
        ...
        updated_estimate = np.array([updated_estimate, params_dict_to_array(self.lower.params)]).max(0)
        self.estimate = np.array([updated_estimate, params_dict_to_array(self.upper.params)]).min(0)
    ...
    def step(self, action):
        ...
        if np.isnan(self.last_snr):
            reward = snr
            self.best_snr = snr
            self.best_solution = self.estimate
    ...
    def render(self, mode='human'):
        print(f"snr: {self.last_snr};    subject: {self.row.subject};     age: {self.row.age};"
              f"    condition: {self.row.condition};     beat_no: {self.selected_beat_ind}")

        # self.axes.clear()
        # self.axes.plot(t, self.target_beat)
        # self.axes.plot(t, sim_beat)
    ...
    def reset(self, row=None, selected_beat_ind=None, mean=False):
        if row is None:
        ...
        if mean:
            ...
            if selected_beat_ind is None:
                self.selected_beat_ind = np.random.choice(np.arange(len(self.row.rel_p1)), 1)[0]
            else:
                self.selected_beat_ind = selected_beat_ind

            self.target_beat = self.set_target_beat(self.row.clean_beats[self.selected_beat_ind])
        ...
        sim_beat = self.action_to_beat(np.zeros_like(params_dict_to_array(self.upper.params)))
